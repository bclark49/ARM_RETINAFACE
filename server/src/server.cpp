...
int main (int argc, char** argv) {
	...
	sprintf (writer_uri, "appsrc name=src blocksize=%d max-bytes=%d max-latency=%d is-live=true caps=\"video/x-raw, width=640, height=480, framerate=%d/1, format=BGR\" ! videoscale n-threads=4 method=0 ! video/x-raw, width=%d, height=%d, format=BGR ! tee name=t \ 
			t. ! queue ! mpph265enc qos=true ! video/x-h265, width=%d, height=%d, stream-format=byte-stream, alignment=au ! queue ! rtph265pay config-interval=1  ! application/x-rtp, media=video, clock-rate=90000, encoding-name=H265, paload=96 ! udpsink host=%s port=9004 max-lateness=%d qos=true sync=false \
			t. ! queue ! fpsdisplaysink video-sink=fakevideosink text-overlay=false signal-fps-measurements=true sync=false name=sink", buffer_size, buffer_size * max_buffers, max_ns, fps, width, height, width, height, host_ip, max_ns);
	/*
	...
	*/
	...
	char reader_uri [512];
	sprintf (reader_uri, "udpsrc port=9002 name=udpsrc caps=\"application/x-rtp, media=video, clock-rate=90000, encoding-name=H265, alignment=au, payload=96\" ! rtph265depay ! video/x-h265, stream-format=byte-stream, alignment=au ! queue ! h265parse config-interval=1 ! queue ! mppvideodec format=16 width=%d height=%d ! queue ! videoscale n-threads=4 method=0 ! video/x-raw, width=640, height=480, format=BGR ! appsink name=sink blocksize=%d max-buffers=%d max-lateness=%d qos=true sync=false", width, height, buffer_size, max_buffers, max_ns);
